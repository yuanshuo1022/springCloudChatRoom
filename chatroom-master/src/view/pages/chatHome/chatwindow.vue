<template>
  <div class="chat-window">
    <div class="top">
      <div class="head-pic">
        <HeadPortrait :src="frinedInfo.headImg"></HeadPortrait>
      </div>
      <div class="info-detail">
        <div class="name">{{ frinedInfo.chatCardName }}</div>
        <div class="detail">{{ frinedInfo.chatCardId }}</div>
      </div>
      <div class="other-fun">
      ...
      <div class="chat-content" ref="chatContent">
        <div class="chat-wrapper" v-for="item in chatList" :key="item.id">
          <div class="chat-friend" v-if="item.uid != userId">
            <div class="info-time">
              <HeadPortrait :src="item.headImg"></HeadPortrait>
...
import { formattedDateTime } from "@/api/getData"
...
export default {
  ...
  mounted() {
    this.getFriendChatMsg();
    //websocket
    this.socket = new WebSocket('ws://localhost:8081/chat1');
    // 监听 WebSocket 连接打开事件
    this.socket.onopen = () => {
      console.log('WebSocket连接已建立', this.frinedInfo.chatCardId);
      // 构造要发送给后端的消息对象
      const data = {
        // 将要传递的值放在这里
        chatId: this.frinedInfo.chatCardId,
      };
      // 将消息对象转换为 JSON 字符串
    ...
    this.socket.onmessage = (event) => {
      ...
      this.chatList.push(message); // Add the received message to the chat list
      this.scrollBottom(); // Scroll to the bottom of the chat window
    };
  },
  methods: {
    //获取聊天记录
    getFriendChatMsg() {
      ...
      // };
      axios.post("http://localhost:9025/friend/getMsg",{"chatId":this.frinedInfo.chatCardId}).then(res => {
        this.chatList = res.data.message;
        console.log(res.data.message.headImg);
        this.chatList.forEach((item) => {
          //item.extend.imgType
          if (item.chatType == 2 && item.extend == 2) {
    ...
    sendText() {
      if (this.inputMsg) {
        console.log("this.frinedInfo.id:" + this.frinedInfo.chatCardId)
        let chatMsg = {
          headImg: require("@/assets/img/head_portrait.jpg"),
          chatIdWindow: this.frinedInfo.chatCardId,
