<template>
  <div class="login-container">
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form" auto-complete="on" label-position="left">
      ...
      <p class="tips">
        <a href="/#/register" style="color: aliceblue;" type="primary">还没有帐号？立即注册</a>
      </p>
...
export default {
  ...
  this.$refs.loginForm.validate(valid => {
    if (valid) {
      ...
      axios.post('http://localhost:9025/api/login', loginData)
        .then(response => {
          this.loading = false;
          // this.$router.push({ path: this.redirect || '/ChatHome' });
      ...
      console.log("result.status:"+result.code+"token"+token)
      this.$message({
              showClose: true,
              message: '登录失败，请重试',
              type: 'error'
