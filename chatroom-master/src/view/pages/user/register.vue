    ...
    methods: {
      sendMsg: function() {
        ...
        if (!emailPass) {
          ...
          console.log(self.ruleForm.email)
        axios.post('http://localhost:9025/api/sendVerCode',{"email":self.ruleForm.email}).then(res => {
            this.$message({
              showClose: true,
      ...
      register: function () {
        this.$refs['ruleForm'].validate((valid) => {
          if (valid) {
            ...
            axios.post("http://localhost:9025/api/register",user).then(res => {
